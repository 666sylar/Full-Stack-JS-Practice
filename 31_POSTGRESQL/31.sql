...
SELECT substr(first_name, 1, 1) || '. ' || last_name as full_name, avg_mark
FROM students
...
SELECT substr(first_name, 1, 1) || '. ' || last_name as full_name, concat(substr(phone_number, 1, 6), '*******') as masked_phone
FROM students;
...
SELECT *
FROM students
WHERE SUBSTR(phone_number, 4, 3) IN ('098', '096', '067');
...
FROM students
WHERE department = 'Information Technology' AND extract(MONTH FROM birthday) BETWEEN 6 AND 8
GROUP BY entered_at
